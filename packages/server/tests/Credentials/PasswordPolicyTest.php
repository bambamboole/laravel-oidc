...
it('verifies and rejects passwords through the credential', function (): void {
    $user = userWithPassword('password');
    $passwords = app(PasswordCredential::class);

    expect($passwords->verify($user, 'password'))->toBeTrue()
        ->and($passwords->verify($user, 'wrong'))->toBeFalse();
});
